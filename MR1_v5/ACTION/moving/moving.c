...
#include "main.h"
#include "path.h"
#define MM_PER_SEC_TO_RPM 2.412857f
...
float arc_pid_outputmax_controller  = 1.0f;
float line_pid_outputmax_controller  = 1.0f;
float V_parallel;
float V_tangent;
...
void moving_init(void)  //***********************************************************************待调
{
	...
	PID_parameter_init(&yaw_pid, 3.1, 0.023, 2.25, 3, 3);// eg1:3, 0.023, 2.25, 2, 2
...
void moving_path_tracking(void)
{
	...
	if((*switch_determine)() == 1)
	{
		...
		// 清除PID误差数据
		PID_clear_error(&arc_pid);
		PID_clear_error(&line_pid);
		PID_clear_error(&point_x_pid);
		PID_clear_error(&point_y_pid);
...
void moving_select_switch_mode_URTUU_TO_RZ(void)
{
	switch(PATH_POINT_COUNTER)
	{
		...
		case 9 : switch_determine = moving_URTUU_TO_RZ_switch_mode_9; break;
		case 10 : switch_determine = moving_URTUU_TO_RZ_switch_mode_10; break;
		case 11 : switch_determine = moving_URTUU_TO_RZ_switch_mode_11; break;
		default : break;
	}
}
...
{
	if((ROBOT_REAL_POS_DATA.X_POS > -500)&&(ROBOT_REAL_POS_DATA.Y_POS > 2500)) return 1;
//	if(2 == 1) return 1;
	else return 0;
}//测试用阈值
*/
...
u8 moving_URTUU_TO_RZ_switch_mode_0(void)
{
	...
	//if(ROBOT_REAL_POS_DATA.Y_POS > 1225) return 1;
...
u8 moving_URTUU_TO_RZ_switch_mode_1(void)
{
	...
	//if(ROBOT_REAL_POS_DATA.Y_POS > 2500) return 1;
	if((ROBOT_REAL_POS_DATA.X_POS > -1225) && (ROBOT_REAL_POS_DATA.Y_POS > 2000)) return 1;
	else return 0;
}
...
{
	if((ROBOT_REAL_POS_DATA.X_POS < -1225) && (ROBOT_REAL_POS_DATA.Y_POS > 3500)) return 1;
	else return 0;
}
...
{
	if(ROBOT_REAL_POS_DATA.Y_POS > 5000) return 1;
	else return 0;
}
...
{
	if(ROBOT_REAL_POS_DATA.Y_POS > 5750) return 1;
	else return 0;
}
...
{
//		if(ROBOT_REAL_POS_DATA.Y_POS > 5750) return 1;
	if(ROBOT_REAL_POS_DATA.X_POS > -1500) return 1;
	else return 0;
}
...
{
	if(ROBOT_REAL_POS_DATA.Y_POS > 8300) return 1;
	else return 0;
}
...
{
	if(ROBOT_REAL_POS_DATA.X_POS < -2000) return 1;
	else return 0;
}
...
{
	if(ROBOT_REAL_POS_DATA.X_POS < -4600) return 1;
	else return 0;
}

u8 moving_URTUU_TO_RZ_switch_mode_9(void)
{
	if(ROBOT_REAL_POS_DATA.Y_POS <= 8500 && ROBOT_REAL_POS_DATA.X_POS < -5775) return 1;
	else return 0;
}
//交接
//////////////////////////////////////////////////
u8 moving_URTUU_TO_RZ_switch_mode_10(void)
{
	if(ROBOT_REAL_POS_DATA.Y_POS > 8600) return 1;
	else return 0;
}
//拾取兽骨
//////////////////////////////////////////////////

u8 moving_URTUU_TO_RZ_switch_mode_11(void)
{
	if(ROBOT_REAL_POS_DATA.X_POS > -4975) return 1;
	else return 0;
}
...
//	static float error_max = 0.0f; 
	...
	//line_pid.outputmax = float_abs(LINE_OUTPUTMAX_CONTROLLER * V_parallel * (error / line_pid.errormax));
...
//	arc_pid.outputmax = float_abs(ARC_OUTPUTMAX_CONTROLLER * V_tangent * (error / arc_pid.errormax));
	...
	V_center = arc_pid.output;					//pid减小d
	//V_center = 0.0f;
	...
	{
	ROBOT_TARGET_VELOCITY_DATA.Vy_RPM = 0;
	ROBOT_REAL_POS_DATA.Y_POS = 8340;
	//ROBOT_TARGET_VELOCITY_DATA.Vx_RPM = 0;
	}
}
...
void moving_yaw_adjusting_time_and_PID(void)
{
	...
	else if((passed_time <= yaw.yaw_t2) && (cal_W_flag == 1))
	{
		...
		{
			if(float_abs(mr1.yaw)+float_abs(yaw.yaw) > 0) error = yaw.yaw - mr1.yaw;
			else error = mr1.yaw - yaw.yaw;
		}
		error = error / 180.0f * PI;
